...
class VideoProcess:
    ...
    def parse_duration(self, duration_str):
        """Parse a duration string (HH:MM:SS or MM:SS) and return total seconds."""
    ...
    def is_video_over_a_lower_b(self, video_id: str, ta: int = 0, tb: int = 120):
        """Check using yt-dlp whether the video duration is over 30 minutes.
        ...
        try:
            ...
            output = subprocess.check_output(
                ["yt-dlp", "--cookies", "../config/cookies.txt", "--get-duration", url],
                stderr=subprocess.STDOUT
            )
            duration_str = output.decode('utf-8').strip()
            total_seconds = self.parse_duration(duration_str)
            print(f"Video {video_id} duration: {duration_str} ({total_seconds} seconds)")
            return total_seconds > ta*60 and total_seconds < tb*60
